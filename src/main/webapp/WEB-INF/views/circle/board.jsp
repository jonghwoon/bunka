...
<body  onload="loadPage('menu_load','menu_top.common');">		
	...
	<c:forEach items="${dtos}" var="board">
     <tr class="light">
		    ...
		    <td><a href = "viewContents.circle?num=${board.num}&subject=${board.subject}&contents=${board.contents}&writer=${board.writer}&readCnt=${board.readCnt}&adddate=${board.adddate}">${board.subject}</a></td>
		    </c:if>
		    <c:if test="${sessionScope.id != board.writer}">
		    <td><a href = "viewContents.circle?num=${board.num}&subject=${board.subject}&contents=${board.contents}&writer=${board.writer}&readCnt=${board.readCnt+1}&adddate=${board.adddate}">${board.subject}</a></td>
		    </c:if>
		  	<td>${board.contents}</td>
		  	<td align="left" width="100px">${board.writer}</td>
		<!--   댓글 갯수 있다면 보여주기 -->
		    <c:if test="${board.recount != 0}">
		   	 <a id="board_comments">[${board.recount}]</a>
		    </c:if>
		    <td align="left">${board.readCnt}</td>
		    
		    <td align="left" width="10%">${board.adddate}</td>
      </tr>
	 </c:forEach>
<!--   테이블 풋(글쓰기 버튼) -->
    </tbody>
		...
		<div align="right" style="margin-right: 30px">
		   <a href="writeForm.circle"><button type="button" class="btn btn-default">글 쓰기</button></a>
		</div>
		</c:if>
	</div>
